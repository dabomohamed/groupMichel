    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("" +
                    "SELECT YEAR(DATASCIENCE_SHARE_DB.SCH_ELABORATE_DATA.BANDE_ENTETE.DATE_MEP_REEL )" +
                    " AS YEAR, MONTH(DATASCIENCE_SHARE_DB.SCH_ELABORATE_DATA.BANDE_ENTETE.DATE_MEP_REEL ) " +
                    "AS MONTH, count(*) FROM DATASCIENCE_SHARE_DB.SCH_ELABORATE_DATA.BANDE_ENTETE " +
                    "WHERE  YEAR  IS NOT NULL AND MONTH IS NOT NULL AND UPPER( ESPECE) " +
                    "NOT LIKE '%POULETTE%' AND UPPER( ESPECE) NOT LIKE '%PLETTE%' " +
                    "GROUP BY YEAR(DATASCIENCE_SHARE_DB.SCH_ELABORATE_DATA.BANDE_ENTETE.DATE_MEP_REEL )," +
                    " MONTH(DATASCIENCE_SHARE_DB.SCH_ELABORATE_DATA.BANDE_ENTETE.DATE_MEP_REEL ) " +
                    "ORDER BY YEAR, MONTH");
